...
static void sunxi_snd_txctrl_i2s0(struct snd_pcm_substream *substream, int on)
{
	...
	if (on) {
		...
		reg_val &= ~SUNXI_I2S0INT_TXDRQEN;
		writel(reg_val, sunxi_i2s0.regs + SUNXI_I2S0INT);
	printk("%d reg_val = 0x%x\n", __LINE__, readl(sunxi_i2s0.regs + SUNXI_I2S0CTL));
	}
}
...
//	u32 read_val;
//	read_val = readl(sunxi_i2s0.regs + SUNXI_I2S0CTL);
//		printk("%d reg_val = 0x%x\n", __LINE__, read_val);
//		read_val &= ~((1 << 0) | (1 << 1) | (1 << 2) | (1 << 3)  | (1 << 8));
//		read_val |= ((0 << 0) | (0 << 1) | (1 << 2) | (0 << 3) | (1 << 8));
//	writel(read_val, sunxi_i2s0.regs + SUNXI_I2S0CTL);
//	printk("%d reg_val = 0x%x\n", __LINE__, read_val);
}
...
//	printk("=====> %s, %s, %d\n", __FILE__,  __FUNCTION__,  __LINE__);
	...
	switch (cmd) {
		...
		case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
			...
			# if 1
//			mdelay(5000);  //测试时用于延时5s后打开BCLK时钟
			/*Global Enable Digital Audio Interface*/
			reg_val = readl(sunxi_i2s0.regs + SUNXI_I2S0CTL);
			reg_val |= SUNXI_I2S0CTL_GEN;  /* 打开BLCK使能 */
	//		reg_val &= ~SUNXI_I2S0CTL_GEN;  /* 关闭BCLK */
//			reg_val |= SUNXI_I2S0CTL_LOOP;
			writel(reg_val, sunxi_i2s0.regs + SUNXI_I2S0CTL);
#endif
			break;
