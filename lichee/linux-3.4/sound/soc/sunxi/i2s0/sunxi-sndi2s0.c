					...
					struct snd_pcm_hw_params *params)
{
...
#else	//Codec SYSCLK select PLL and PLLCLK source select  BCLK=4096000

	ret = snd_soc_dai_set_sysclk(codec_dai, 1 , freq, i2s0_pcm_select);
	if (ret < 0) {
		return ret;
	}

	/*set pll*/
	ret = snd_soc_dai_set_pll(codec_dai, 1, 0, 4096000, freq);
	if (ret < 0) {
		pr_warn("[daudio0],the codec_dai set set_pll failed.\n");
		return ret;
	}
	...
	ret = snd_soc_dai_set_fmt(codec_dai, (audio_format | (signal_inversion<<8) | (i2s0_master<<12)));
	if (ret < 0) {
		pr_err("%s, line:%d\n", __func__, __LINE__);
		return ret;
	}
...
static struct snd_soc_dai_link sunxi_sndi2s0_dai_link = {
	...
	.codec_name		= "ac108.0-003b",//"ac108.2-003b",
#else
