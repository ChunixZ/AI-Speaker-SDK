...
static script_item_u pa_power;
#define PA_POWER_0    0
#define PA_POWER_1    1
#define PA_POWER_MSLEEP    300
...
void head_set_pa(int val)
{
	if(val == 1){
//		printk("===========jeffrey_headset: in_1\n");
		gpio_set_value(item.gpio.gpio, 1);
	}else{
//		printk("===========jeffrey_headset: out_1\n");
		gpio_set_value(item.gpio.gpio, 0);
	}
}
EXPORT_SYMBOL(head_set_pa);
...
//			gpio_set_value(item.gpio.gpio, 1);
		...
		if (play_running == 0) {
			gpio_set_value(item.gpio.gpio, 0);
			msleep(PA_POWER_MSLEEP);
		    gpio_set_value(pa_power.gpio.gpio, PA_POWER_0);
	...
	if (current_running == 0) {/*play stream*/
		...
		if(mute == 0) {
			...
			case 1:
			    ...
			    gpio_set_value(pa_power.gpio.gpio, PA_POWER_1);
				msleep(PA_POWER_MSLEEP);
				usleep_range(2000, 3000);
				gpio_set_value(item.gpio.gpio, 1);

				codec_wr_prcm_control(DAC_PA_SRC, 0x1, 2, 0x1);
			    codec_wr_prcm_control(DAC_PA_SRC, 0x1, 3, 0x1);
			...
			default:
				...
				codec_wr_prcm_control(DAC_PA_SRC, 0x1, 2, 0x0);
				codec_wr_prcm_control(DAC_PA_SRC, 0x1, 3, 0x0);
		}
	...
	struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		/* I2S0 TX DISABLE *///2017-09-11qin
//		codec_wr_control(SUNXI_DA_CTL ,  0x1, TXEN,0);
		/*SDO ON*///2017-09-11qin
...
static int __devinit sndpcm_codec_probe(struct platform_device *pdev)
{
	...
	*/
	...
	type = script_get_item("audio0", "audio_power_ctrl", &pa_power);
	if (SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
		pr_err("[ audio ] err:try to get audio_power_ctrl failed!\n");
		return -EFAULT;
	}
	req_status = gpio_request(pa_power.gpio.gpio, NULL);
	if (0 != req_status) {
		pr_err("request gpio failed!\n");
	}
	gpio_direction_output(pa_power.gpio.gpio, 0);
	gpio_set_value(pa_power.gpio.gpio, 0);
	...
	gpio_direction_output(item.gpio.gpio, 0);
	gpio_set_value(item.gpio.gpio, 0);
