...
#define AC108_PGA_GAIN				0x38	// 0x3d //range[0, 0x3f], eg: 0->0dB, 0x3d->30.5dB
#define AC108_REF_PGA_GAIN		0x3d	// 0x10	//range[0, 0x3f], eg: 0->0dB, 0x3d->30.5dB
//2017-09-12 qin 
//#define AC108_PGA_GAIN			0x3d	//range[0, 0x3f], eg: 0->0dB, 0x3d->30.5dB
//#define AC108_REF_PGA_GAIN		0x34	//range[0, 0x3f], eg: 0->0dB, 0x3d->30.5dB
...
{
	/*** Chip reset ***/
	//ac108_write(CHIP_AUDIO_RST, 0x12, i2c);	/*0x00=0x12: resets all register to their default state*/
	...
	/*** I2S Common Config ***/
	ac108_update_bits(I2S_CTRL, 0x1<<SDO1_EN | 0x1<<GEN, 0x1<<SDO1_EN | 0x1<<GEN, i2c);	/*SDO1 enable, Globe Enable*/
	...
	ac108_write(I2S_FMT_CTRL3, 0x70, i2c);	/*0x36=0x70: TX MSB first, TX2 Mute, LRCK = 1 BCLK width (short frame), Linear PCM Data Mode*/
											//Transfer 0 after each sample in each slot(sample resolution < slot width), 
	...
	if (i2c->addr == 0x35) {
		ac108_write(ANA_PGA1_CTRL, AC108_PGA_GAIN, i2c);	/*0x90=0x3d: ADC1 PGA gain 30.5dB*/
		ac108_write(ANA_PGA2_CTRL, AC108_PGA_GAIN, i2c);	/*0x91=0x3d: ADC2 PGA gain 30.5dB*/
		ac108_write(ANA_PGA3_CTRL, AC108_REF_PGA_GAIN, i2c);	/*0x92=0x3d: ADC3 PGA gain 30.5dB*/
		ac108_write(ANA_PGA4_CTRL, AC108_REF_PGA_GAIN, i2c);	/*0x92=0x3d: ADC3 PGA gain 30.5dB*/
		//ac108_write(ANA_PGA4_CTRL, AC108_PGA_GAIN, i2c);	/*0x93=0x3d: ADC4 PGA gain 30.5dB*/
	} else if (i2c->addr == 0x3b) {
		ac108_write(ANA_PGA1_CTRL, AC108_PGA_GAIN, i2c);	/*0x90=0x3d: ADC1 PGA gain 30.5dB*/
		ac108_write(ANA_PGA2_CTRL, AC108_PGA_GAIN, i2c);	/*0x91=0x3d: ADC2 PGA gain 30.5dB*/
		ac108_write(ANA_PGA3_CTRL, AC108_PGA_GAIN, i2c);	/*0x92=0x3d: ADC3 PGA gain 30.5dB*/
		ac108_write(ANA_PGA4_CTRL, AC108_PGA_GAIN, i2c);	/*0x93=0x3d: ADC4 PGA gain 30.5dB*/
	}
...
static int ac108_set_clkdiv(struct snd_soc_dai *dai, int div_id, int div)
{
	...
	bclk_div = div/(AC108_LRCK_PERIOD * (AC108_ENCODING_EN ? (AC108_ENCODING_CH_NUMS+1)/2 : 1));		//default PCM mode
	//bclk_div = div/(2*AC108_LRCK_PERIOD * (AC108_ENCODING_EN ? (AC108_ENCODING_CH_NUMS+1)/2 : 1));		//I2S/LJ/RJ mode
...
#if !AC108_ENCODING_EN		//TX Encoding mode wil config BCLK_DIV at trigger function
	...
	switch(cmd){
		...
		case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
			if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
				...
				ac108_multi_chips_update_bits(I2S_CTRL, 0x1<<RXEN, 0x1<<RXEN);
			}
#if 0
			AC108_ENCODING_EN	//TX Encoding mode must supply BCLK after AC108 config completely(enable TX block then count BCLK), and disable BCLK at hw_free
			ac108_multi_chips_update_bits(I2S_BCLK_CTRL, 0xf<<BCLKDIV, ac108_bclk_div_val<<BCLKDIV);
#endif
			break;
...
static int ac108_fetch_sysconfig_para(void){
        int ret = -1;
        u32   ac108_used_flag;
        script_item_value_type_e  type;
        script_item_u ac108_item_temp;
        struct gpio_config ac108_power_gpio;
       
	type = script_get_item("ac108_para", "ac108_used", &ac108_item_temp);
	if(type == SCIRPT_ITEM_VALUE_TYPE_INT){
		ac108_used_flag = ac108_item_temp.val;
	}else{
		printk("script_parser_fetch ac108_used_flag failed\n");
		ac108_used_flag = 0;
	}
        
 ///////////////////////////////////////////////////////////////     
    //if(ac108_used_flag){
        if (1 != ac108_used_flag ) {
        	printk(KERN_ERR"%s: ac108_unused. ac108_used_flag = %d\n",  __func__, ac108_used_flag);
        }else{
        	printk("ac108_used_flag = %d\n", ac108_used_flag);
        	}
        
 ////////////////////////////ac108_power_on///////////////////////////////////       
        type = script_get_item("ac108_para", "ac108_power_on", &ac108_item_temp);
        if (SCIRPT_ITEM_VALUE_TYPE_PIO == type) {
        	printk("get ac108_power_on gpio OK!\n");
        	ac108_power_gpio.gpio = ac108_item_temp.gpio.gpio;        
        } else {
             printk("get ac108 power gpio failed\n");
        }       
                
          /* 
     * 申请GPIO使能 设置输出高电平 
     */
	ret = gpio_request(ac108_power_gpio.gpio, "ac108_power_on");
	if (ret < 0) {
		printk("Failed to request gpio [%d] for ac108_power_on\n", ac108_power_gpio.gpio);
		gpio_free(ac108_power_gpio.gpio);
		return ret;
	}
	else
		{
			printk("request gpio [%d] for ac108_power_on OK!\n",ac108_power_gpio.gpio);
			}

	if (0 != gpio_direction_output(ac108_power_gpio.gpio, 1)){
		pr_err("ac108 power gpio set err!");
	}
	__gpio_set_value(ac108_power_gpio.gpio, 1);
        printk("ac108_power_gpio.gpio = %d\n",__gpio_get_value(ac108_power_gpio.gpio));
...
static int ac108_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *i2c_id)
{
	...
	if (!regulator_en) {
		...
		printk("%d %s ret = %d", __LINE__, __func__, ret);
		if (ret) {
	...
	if (i2c_id->driver_data == 0) {
		ret = snd_soc_register_codec(&i2c->dev, &ac108_soc_codec_driver, &ac108_dai0, 1);//(struct snd_soc_dai_driver *)
		i2c_clt[0] = i2c;
	} else if (i2c_id->driver_data == 1) {
		ret = snd_soc_register_codec(&i2c->dev, &ac108_soc_codec_driver, &ac108_dai1, 1);//(struct snd_soc_dai_driver *)
		i2c_clt[1] = i2c;
	} else {
		pr_err("The wrong i2c_id number :%d\n", (int)(i2c_id->driver_data));
	}
