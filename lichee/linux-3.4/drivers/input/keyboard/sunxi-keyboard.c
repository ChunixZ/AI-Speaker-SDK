...
static u32 debug_mask = 8;
#define debug_printk(level_mask, fmt, arg...)	if (unlikely(debug_mask & level_mask)) \
...
static void sunxikbd_clk_cfg(void)
{
	...
	debug_printk(DEBUG_INIT, "%s: get key_clk_source rate %dHZ\n", __func__, (__u32)rate);
...
static void sunxi_keyboard_early_suspend(struct early_suspend *h)
{
	...
	debug_printk(DEBUG_SUSPEND, "[%s] enter standby state: %d. \n", __FUNCTION__, (int)standby_type);
...
static void sunxi_keyboard_late_resume(struct early_suspend *h)
{
	...
	debug_printk(DEBUG_SUSPEND, "[%s] return from standby state: %d. \n", __FUNCTION__, (int)standby_type); 
...
//printk("reg_val = %d\n", reg_val);
	...
	if (reg_val & LRADC_ADC0_DATAPEND) {
		...
		if ((key_cnt + 1) < REPORT_START_NUM) {
			...
			if (1 == judge_flag) {
	//			debug_printk(DEBUG_INT, "report data: key_val :%8d transfer_code: %8d , scancode: %8d\n", \
	//				key_val, transfer_code, scancode);
				...
				if (transfer_code == scancode) {
					...
					input_sync(sunxikbd_dev);
					transfer_code = scancode;
					printk("report data: key_val = %d %d\n", sunxi_scankeycodes[transfer_code], __LINE__);
				}
			}
		}
	}
...
//			debug_printk(DEBUG_INT, "report data: key_val :%8d transfer_code: %8d \n",key_val, transfer_code);
...
printk("%s %d\n", __FUNCTION__,  __LINE__);
 	debug_printk(DEBUG_INIT, "sunxikbd_init \n");
...
#ifdef CONFIG_HAS_EARLYSUSPEND 
	debug_printk(DEBUG_INIT, "==register_early_suspend =\n");
...
#endif
	debug_printk(DEBUG_INIT, "sunxikbd_init end\n");
