...
extern "C" {
#endif
int ac108_control_init();
int ac108_read(char* bu_buf);
#ifdef __cplusplus
}
#endif
...
void DCSApplication::run() {
    ...
    while (true) {
        ...
        ac108_read(readBuf);
        m_dcsSdk->writeAudioData(readBuf, 640);
        if (fp == nullptr) {
            snprintf(filename, sizeof(filename), "./%d.pcm", count);
            fp = fopen(filename, "a");
            if (fp) {
                fwrite(readBuf, 1, 1280, fp);
                count++;
                loop++;
            }
        } else {
