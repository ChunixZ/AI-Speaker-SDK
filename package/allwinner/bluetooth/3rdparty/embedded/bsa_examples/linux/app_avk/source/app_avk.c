...
static int send_azi = 0, timeout_count = 0;
static void send_to_az(void)
{
	printf("%s >>>>>>[%d]\n",__func__,__LINE__);
	int res = -1, send_b = sstop_az, i = 0;
	...
	if(res != -1)
	printf("connect socket success avk_app\n");

	res = send(sockfd,&send_b,sizeof(send_b),0);
...
tBSA_AVK_MSG *p_data_az;
tAPP_AVK_CONNECTION *connection_az;

static void app_avk_handle_start_az(void)
{
	tBSA_AVK_MSG *p_data = p_data_az;
	tAPP_AVK_CONNECTION *connection = connection_az;
...
static void app_avk_cback(tBSA_AVK_EVT event, tBSA_AVK_MSG *p_data)
{
    ...
    case BSA_AVK_START_EVT:
        APP_DEBUG1("BSA_AVK_START_EVT status 0x%x, streaming ? %d,send_azi=%d", p_data->start.status, p_data->start.streaming,send_azi);
...
/* close alsa dev */
    ...
    case BSA_AVK_RC_OPEN_EVT:
		printf("[%d]  BSA_AVK_RC_OPEN_EVT %d\n",__LINE__,p_data->rc_open.status);
